<?php
	...
	else if($_POST['service'] == 'register')
	{
		...
		try
		{
			...
			$body .= '<img src="' .$message->embed(Swift_Image::fromPath('../../rsvp/irresistiblepackage/images/email/EmailHeader.png')) . '" /><br><br><br>';
	...
	else if($_POST['service'] == 'submit_offers')
	{
		...
		if($info['eventID'] != '')
		{
			...
			if($event['dealerID'] == 406 || $event['dealerID'] == 12106){
				$sql = 'SELECT * FROM ps_dealerstaff WHERE status = 1 AND dealerID= ' . $event['dealerID'];
				$results = mysqli_query($db_data,$sql);
				while($re = mysqli_fetch_assoc($results))
				{
					if($re['webContact'] == 'on' && $re['email'] != "") $webLeadContacts[] = trim($re['email']);
				}
			}

			$to = $webLeadContacts;
			$bcc = array('emeerstra@absoluteresults.com', 'jdignall@absoluteresults.com');
			$from = 'web@absoluteresults.com';
			$subject = "Customer Selected Irresistible Package Offers";
			$body = "First Name: " . $info['customer']['firstname'] .'<br>';
			$body .= "Last Name: " . $info['customer']['lastname'] .'<br>';
			$body .= "Email: " . $info['customer']['email'] .'<br><br>';
			$body .= "Offers: <br>";
