<?php
	...
	if($_GET['fromContainer'] != '') $xls->send(str_replace(" ","",str_replace(" ","",$_SESSION['container']['dealerName'] . '-' . mysqli_num_rows($results)) . '-' . (isset($_GET['email_to_mail']) ? 'E2M-' : '') . ($showNum ? 's-' : '') . 'SalesRepPhoneList') . '.xls');
		...
		foreach($list as $cus)
		{
			...
			foreach($columnsArray as $column)
			{
				...
				else if(in_array($column['column'],array("mainPhone","businessPhone","mobilePhone")) && trim($cus[$column['column']]) != "" && ($cus[$column['column'].'NDNC'] == 'yes' || $cus[$column['column'].'NDNC'] == '2'))
				{
					...
					if(!($cus['lastPaymentDate'] >= date("Y-m-d") || $cus['serviceDate'] >= date("Y-m-d",strtotime("now - 17 months")) || $cus['deliveryDate'] >= date("Y-m-d",strtotime("now - 17 months"))))
					{
