<?php
	...
	if(!empty($_GET['hash'])) {
		$agreementIDs = [];
		if($_GET['bundle']) {
			$bundle = WorksheetBundle::byHash($_GET['hash']);

			if($bundle instanceof WorksheetBundle) {

				$worksheets = $bundle->worksheets;
			...
			if(!empty($hashes)) {
				$results = Worksheet::where('hash',$hashes,'IN')->get();
				if(!empty($results)) {
			...
			if(count($agreementIDs) != count($hashes)) {
				echo 'Invalid Agreement';
				exit;
			}
		}
		...
		if(empty($_GET['aids']) && empty($_GET['id'])) {
			echo 'Invalid Agreement';
			exit;
