...
session_start('conference');
	...
	$dataTableJSPath = AR_URL."scripts/DataTables/media/js/";
	$dataTableExportJSPath = AR_URL."scripts/DataTables/media/js/exportjs/";
	$dataTableCSSPath = AR_URL."scripts/DataTables/media/css/";
	$dataTableExportCSSPath = AR_URL."scripts/DataTables/media/css/exportcss/";
...
<script>
	...
	function loadTable(){
		<?php
			$conf = new TrainerConference2017();

			$rows = TrainerConference2017::get();
			/*if($_POST['report'] == 1){
				$galaRows = $gala->get(null);
			}
			else{*/
				$rows = array();
			//}
	...
	<div class="container">
		...
		<form id="rsvpForm" method="POST">
			<select id="report" name="report">
				<option></option>
				<option value="1" <?= ($_POST['report'] == 1 ? 'SELECTED' : '' ) ?> >RSVP List</option>
			</select>
			<!-- <input type="submit" value="Get Report" > -->
			<a href="#" onClick="submitForm(); return false;" style="margin-left: 15px; padding: 5px 10px; background-color:#1E90FF; border-radius: 3px; color: white; font-weight: bold;">
				GetReport
			</a>
		</form>
